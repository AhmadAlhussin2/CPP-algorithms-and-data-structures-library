...
def visualize_general():
    ...
    for data_type, algorithms in data.items():
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f"charts/{data_type}_{category}_benchmark.png", dpi=1200)
        plt.close()


def visualize_ordered_containers_results():
    # Parse and organize benchmark entries
    for entry in benchmarks['benchmarks']:
        name = entry["name"]
        time = entry["real_time"]
        ...
        data_type, algorithm, variation, input_size = match.groups()
        input_size = int(input_size)
        data[variation][algorithm][data_type].append((input_size, time))
    ...
    # Generate plots
    for variation, algorithms in data.items():
        plt.figure(figsize=(10, 6))
        plt.title(f"{data_type} - Benchmark Results")
        plt.xlabel("Input Size")
        plt.ylabel("Real Time (ms)")
        ...
        for algorithm in algorithms:
            linestyle = next(linestyles)
            for _, values in algorithms[algorithm].items():
                values.sort()
                x, y = zip(*values)
                plt.plot(x, y, label=f"{variation} / {algorithm}",
                         linestyle=linestyle,
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f"charts/{variation}_{category}_benchmark.png", dpi=1200)
        plt.close()


if category == 'ordered_containers_results':
    visualize_ordered_containers_results()
else:
    visualize_general()
